...
### Session 2025-10-28（分析回饋對齊）
- 偏好調整的可見性：N 與相似度門檻（τ）為使用者可調參數，需提供清楚的介面（設定或等效入口）以便調整並即時生效。
- 行為記錄之隱私：對「接受/移除建議」的行為記錄僅存於裝置端，用於後續提升建議品質；不會上傳或分享，並採有限保存策略（例如保留 90 天或最近 1,000 筆，以先到者為準）。
...
- **FR-010**: 系統必須記錄使用者對建議關聯的接受/移除行為（僅用於後續改善建議品質；不得暴露個資）。
  - 記錄僅儲存在裝置端、不對外傳輸；採有限保存策略（例如保留 90 天或最近 1,000 筆，以先到者為準）；使用者可於設定中檢視/清除記錄。
...
- **FR-012**: 建議清單上限 N 之預設值為 10，且可由使用者在 5–20 範圍內調整；調整介面需清楚，並確保變更即時生效與持久化。
  - 必須提供使用者可見之調整入口（例如：設定頁或等效位置），並以說明文字提示預設值與影響。
...
- **FR-017**: 手動建立關聯之顯示與排序（Pinned）：使用者手動建立的 A↔B 關聯必須「固定顯示且優先」於相關清單頂端，不受相似度門檻（τ）限制，且「不佔用」Top‑N 配額（N）；Pinned 區塊與演算法建議區塊的分組/樣式需清楚可辨。移除手動關聯即取消 Pinned 身份，之後是否出現在建議清單由演算法與門檻決定。
  - 可用性與無障礙：Pinned 項目需具明顯視覺區隔（如徽章/分組標題）與語音提示（例如 VoiceOver 讀出「Pinned」）。
